...
export default function AddressPanel({ onClose }) {
  ...
  const handleAdd = () => {
    ...
    if (addresses.length >= 5)
      return alert("배송지는 최대 5개까지 등록 가능합니다.");
  ...
  const handleSetDefault = (id) => {
    setAddresses((prev) =>
      updateDefaultAndSort(prev.map((a) => ({ ...a, isDefault: a.id === id })))
    );
  ...
  const AddressCard = ({ addr, showActions = false }) => (
    <div
      className={`p-4.5 rounded-lg bg-white border ${
      ...
      {showActions && (
        <div className="flex space-x-2 mt-2">
          ...
          <Button
            ...
            onClick={() => {
              if (addr.isDefault)
                toast.error("대표 배송지는 삭제할 수 없습니다.");
