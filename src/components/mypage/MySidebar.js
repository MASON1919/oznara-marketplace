...
export default function DashboardWithSidebar() {
  ...
  return (
    <div className="flex">
      ...
      {/* 각 패널 */}
      {openPanel === "sales" && (
        <SalesPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "purchases" && (
        <PurchasesPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "delivery" && (
        <DeliveryPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "wishlist" && (
        <WishlistPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "escrow" && (
        <EscrowPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "bank" && <BankPanel onClose={() => setOpenPanel(null)} />}
      {openPanel === "address" && (
        <AddressPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "reviews" && (
        <ReviewsPanel onClose={() => setOpenPanel(null)} />
      )}
      {openPanel === "delete" && (
        <DeleteAccountPanel onClose={() => setOpenPanel(null)} />
      )}
    </div>
  );
}
